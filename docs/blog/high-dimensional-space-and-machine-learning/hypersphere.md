...
> 核心结论：embedding 呈现近似超球面分布，通常是三种机制共同作用的结果：高维统计先导致范数集中，归一化与对比式训练目标进一步压缩径向自由度，而内积型读出又持续奖励方向可读的组织方式；因此，在大量任务中，角度比长度更稳定，也更接近表示真正承载的语义结构 [1-5]。

在“高维空间与机器学习”系列中，本文收束前三篇的主线：前两篇先说明距离为何退化、方向为何稳定，这里则解释训练后的 embedding 为什么常被进一步压到近似球壳上；若继续追问 LLM 词表如何在这种几何上组织，可接着读 [Embedding 空间中的语义线性结构](/blog/representation-space-of-large-models/semantic-linearity)。
...
更准确地说，**cosine similarity 之所以在 embedding 系统中反复奏效，不只是因为它方便，而是因为训练后的表示本来就越来越接近球面上的几何。** 这也为后文讨论球面码、词表容量与大模型 embedding 的组织方式提供了直接前提。

上一篇：[高维向量近似正交的几何机制](/blog/high-dimensional-space-and-machine-learning/orthogonality)。延伸阅读：[Embedding 空间中的语义线性结构](/blog/representation-space-of-large-models/semantic-linearity)。
